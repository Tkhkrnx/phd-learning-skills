...
4. 生成可支撑 PPT 讲述的七问阅读笔记；审稿时同时生成五问加书写细节的分析笔记，以及含优势、缺陷、作者问题和最终推荐的正式意见
5. 核验 LaTeX 参考文献，并生成中文参考文献验证报告 PDF

阅读与审稿 Skill 不再要求 PaperQuay ID、MinerU 缓存或已有笔记。用户与老师商讨后的最新总结决定表达重点，PPT 其次，较早笔记再次；论文原文负责核对事实。审稿的两份文稿必须在优势、缺陷、证据强度和推荐结论上保持一致。所有产物均保留源文件，不凭空编造用户曾有的误解。
