...
- `targeted-knowledge-closure`
  - 扮演懂内容也懂教学法的导师
  - 可以先讲解，并按当前任务需要做纠偏和迁移，不强制固定轮次或测验
  - 重点收敛到：
    - 准确心智模型和前置知识修复
    - 用自己的话独立重构
    - 与近似概念或常见误解进行辨析
...
专家可以先给出完整的候选问题陈述、若干可行方案、系统模型或示范讲解，用户不必先从空白开始构造。只有当用户掌握的观察、意图或取舍会实质改变结果时，Skill 才提出聚焦问题；信息已经足够时应继续推进。仍有关键不确定性时保留暂定状态，并在退出时说明其影响。阶段名、状态和生命周期标记只在内部维护。直接写作、审阅、同步、编码、调试、跑实验、修 replay 或执行已冻结方案走普通执行流程。
...
协作以资料调查为先：AI 解释已有认识、假设和反证，再围绕真正的不确定性提问；用户的纠正必须改变下一轮检索或判断。方法设计从“问题 → 现有方法失败 → 核心挑战 → 针对性启发 → 组合方案 → 判别证据”展开，通常可整理为三个挑战但不凑数。研究对象限定在系统、体系结构与大模型推理/服务方向，启发可来自其他领域；具体方法由问题决定，不强制提出新系统机制。

最终输出必须继承用户原稿或指定几问的标题、顺序、语言和用途。摘要级背景与定义采用紧凑论文段落，讨论说明与正式表达分别处理。写作、总结和交接不得把已确认问题改成求解问句，也不得新增未经讨论的失败原因。

问题定义的每次候选、总结和交接都必须陈述证据支持的状态或限制，例如“人在水下不能自然呼吸”。“如何用潜水装置呼吸”“研究如何优化装置”“缺少我们提出的装置”均不能替代问题。方法设计入口也检查这一点。交流问题和实验 RQ 可以使用问句；问题定义本身必须回答 what the problem is、why it matters、why existing work still fails，且不能把陈述句形式当作学术新颖性的证明。
...
4. 阅读论文时生成可支撑 PPT 讲述的七问笔记；审稿工作分两阶段：先生成可直接用于准备 PPT 的分析笔记，汇报和讨论后再根据 PPT、讨论结论及用户修改过的分析笔记生成正式审稿意见
5. 核验 LaTeX 参考文献，并生成中文参考文献验证报告 PDF

阅读与审稿 Skill 不再要求 PaperQuay ID、MinerU 缓存或已有笔记。阅读笔记前五问按“问题—重要性—现有工作缺口—核心想法—设计”形成讲述主线，实验设置和图表证据另列，第六问整理写作与内容细节，第七问总结证据支持的结论。审稿分析使用不同的五问：“问题定义—重要性—现有工作—核心想法—实验是否支撑结论”，每一问都要写出评审判断；实验设置与图表证据地图放在第五问内，第六问检查写作、术语与展示细节，并区分影响接收判断的问题和次要修订，最后给整体判断。

审稿 Skill 第一阶段只生成 PPT 准备分析笔记；第二阶段在用户汇报和讨论后，使用其 PPT、讨论或导师意见、以及用户修改过的分析笔记生成正式审稿意见。阶段之间由用户自行推进，不强制插入与模型的讨论或确认回合。最新讨论结论决定评审立场和重点，论文原文负责核实事实；正式意见须保持优势、缺陷、证据强度和推荐结论一致。所有产物均保留源文件，不凭空编造用户曾有的误解。
